...
import { API_CONFIG, NASA_API_CONFIG } from "@/shared/config/api/api-config";
...
export const nasaAxiosInstance = axios.create(NASA_API_CONFIG);
...
export const callNasaApi = async <Success, ErrorShape = ApiErrorResponseRecord>(
  config: AxiosRequestConfig,
): Promise<Success> => {
  try {
    const { data } = await nasaAxiosInstance.request<Success>(config);
